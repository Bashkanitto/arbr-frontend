...
export const WithdrawsPageTable = () => {
  ...
  const handleEditSave = (updatedItem: (typeof elementsData)[0]) => {
    setElementsData(elementsData.map(item => (item.id === updatedItem.id ? updatedItem : item)))
  }
  ...
  const renderRow = () => {
    ...
    return filteredData.map((item, index) => (
      <Table.Tr key={index}>
        ...
        <Table.Td>
          <DateItem variantColor={getStatusColor(item.status)}>{item.status}</DateItem>
        </Table.Td>
        <Table.Td>{`${item.sum}₸`}</Table.Td>
        <Table.Td>{format(item.date, 'dd MMMM, yyyy', { locale: ru })}</Table.Td>
            ...
            <Table.Tr>
              <Table.Th>
                <Checkbox
                  ...
                  indeterminate={selectRows.length > 0 && selectRows.length < elementsData.length}
